...
describe('MovieTestService', () => {
  ...
  it('should add a new movie to the movie list (2)', () => {
    const service: MovieUtilService = TestBed.get(MovieUtilService);
    service.addMovie({
      title: 'Some Movie',
      genre: 'Some genre',
    ...
    expect(service.getMovies().length).toEqual(4);
  });
  ...
  it('should NOT update a movie in the movie list', () => {
    const service: MovieUtilService = TestBed.get(MovieUtilService);
    const newMovie = {
      title: 'what',
      genre: 'Some genre',
    ...
    service.update(newMovie);
    expect(service.getMovie(0).title).toEqual('Uncharted');
  });
});
