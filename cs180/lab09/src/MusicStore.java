...
public class MusicStore {
    ...
    public MusicStore() {
        ...
        this.storeProducts = new Product[this.maxProducts];
    ...
    public Product[] filterAudioOrVideo(boolean audio) {
       ...
       for (int i = 0; i < this.totalProducts; i++) {
           if (audio == true) {
               if (this.storeProducts[i] instanceof  AudioProduct) {
                   filteredProducts[n] = this.storeProducts[i];
                   n++;
               }
           } else {
               if (this.storeProducts[i] instanceof VideoProduct) {
                   filteredProducts[n] = this.storeProducts[i];
                   n++;
               }
           }
    ...
    public Product[] filterDownloadable(boolean downloadable) {
        ...
        for (int i = 0; i < this.totalProducts; i++) {
            if (downloadable == true) {
                if (this.storeProducts[i] instanceof  Downloadable) {
                    filteredProducts[n] = this.storeProducts[i];
                    n++;
                }
            } else {
                if (this.storeProducts[i] instanceof Downloadable == false) {
                    filteredProducts[n] = this.storeProducts[i];
                    n++;
                }
            }
